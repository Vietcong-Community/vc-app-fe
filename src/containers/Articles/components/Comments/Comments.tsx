...
import { useAddArticleComment, useArticleComment } from '../../../../api/hooks/articles/api';
...
export const ArtComments: React.FC<IProps> = (props: IProps) => {
  ...
  const comments = useArticleComment(articleId, { page: selectedPage });
    ...
    <>
      <EaseInOutContainer isOpen={!comments.isLoading}>
        <S.Card>
          ...
          <Gap defaultHeight={8} />
          <Divider />
          {(comments.data?.comments?.length === 0 || comments.isError) && (
            <S.NoCommentsText>
