...
import { useMatchById } from '../../../api/hooks/mixedLeague/match/api';
...
import * as S from './MixedMatchDetail.style';

export const MixedMatchDetail: React.FC = () => {
  const { query } = useRouter<{ id: string }>();

  const matchDetail = useMatchById(query.id);

  const scoreExists =
    matchDetail.data?.status !== MixedMatchStatus.NEW && matchDetail.data?.status !== MixedMatchStatus.READY;
